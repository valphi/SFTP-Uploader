...
REM Copy Windows batch files to root directory
copy run-scripts-windows\run_windows.bat .
copy run-scripts-windows\schedule_windows.bat .
copy run-scripts-windows\remove_schedule_windows.bat .
...
:: Create the zip file
powershell -Command ^
    "Compress-Archive -Path 'run_windows.bat', 'schedule_windows.bat', 'remove_schedule_windows.bat', 'SftpClient.jar', '%JRE_TEMP_FOLDER%', 'docx-readme\README_WINDOWS.docx', 'pdf-readme\README_WINDOWS.pdf' -DestinationPath '%ZIP_FILE%' -Force"
...
REM Remove Windows batch files from root directory
del run_windows.bat
