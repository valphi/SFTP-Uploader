...
# Define variables
ZIP_FILE="archive_ubuntu.zip"
JRE_ORIGINAL_FOLDER="jre_21_linux_amd64" # Ubuntu-specific JRE folder
JRE_TEMP_FOLDER="jre"

# Copy Ubuntu shell files to root directory
cp run-scripts-ubuntu/run_ubuntu.sh .
cp run-scripts-ubuntu/schedule_ubuntu.sh .
cp run-scripts-ubuntu/remove_schedule_ubuntu.sh .
...
cp pdf-readme/README_UBUNTU.pdf README_UBUNTU.pdf

# Create the zip file
zip -r "$ZIP_FILE" run_ubuntu.sh schedule_ubuntu.sh remove_schedule_ubuntu.sh SftpClient.jar "$JRE_TEMP_FOLDER" user_indicator macro_indicator portfolio README_UBUNTU.md README_UBUNTU.pdf
if [ $? -eq 0 ]; then
...
rm README_UBUNTU.pdf

# Remove Ubuntu shell files from root directory
rm run_ubuntu.sh
rm schedule_ubuntu.sh
rm remove_schedule_ubuntu.sh
